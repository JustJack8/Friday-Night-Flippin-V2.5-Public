...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		logoBlx = new FlxSprite(0, -20);
		logoBlx.frames = Paths.getSparrowAtlas("logoBumpinFlip");
		logoBlx.antialiasing = true;
		logoBlx.animation.addByPrefix('bump', 'logo bumpin', 24);
		logoBlx.animation.play('bump');
		logoBlx.updateHitbox();
		logoBlx.alpha = 0;
		...
		switch(easterEgg.toUpperCase())
		{
			...
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
				gfDance.frames = Paths.getSparrowAtlas('flippinDanceTitle');
	...
	override function beatHit()
	{
		...
		if(logoBlx != null) 
			logoBlx.animation.play('bump', true);

		if(logoBl != null)
		{
			if (logoBl.alpha == 0)
			{
				FlxTween.tween(logoBl, {alpha: 1,angle: 0}, tweentime, {ease: FlxEase.quintOut});
				FlxTween.tween(logoBlx, {alpha: 0,angle: 0}, tweentime, {ease: FlxEase.quintOut});
			}
			else
			{
				FlxTween.tween(logoBl, {alpha: 0,angle: angleRight}, tweentime, {ease: FlxEase.quintOut});
				FlxTween.tween(logoBlx, {alpha: 1,angle: angleRight}, tweentime, {ease: FlxEase.quintOut});
			}
		}
