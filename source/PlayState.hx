...
class PlayState extends MusicBeatState
{
	...
	public var sicksval:Int = 0;

	public var goodcounter:FlxText;
	public var goodsval:Int = 0;

	public var badcounter:FlxText;
	public var badsval:Int = 0;

	public var shitcounter:FlxText;
	public var shitsval:Int = 0;
	...
	public function bruhevent(cum)
	{
		...
		if (cum == 1)
		{
			bruh.loadGraphic(Paths.image('ballsHD/gay1'));
		}
		else if (cum == 2)
		{
			bruh.loadGraphic(Paths.image('ballsHD/gay2'));
		}
		else if (cum == 3)
		{
			bruh.loadGraphic(Paths.image('ballsHD/gay3'));
		}
		else if (cum == 4)
		{
			bruh.loadGraphic(Paths.image('ballsHD/gay4'));
		}
		else if (cum == 5)
		{
			bruh.loadGraphic(Paths.image('ballsHD/gay5'));
		}
	...
	override public function create()
	{
		...
		{
			case 'sen':
				if(Assets.exists("assets/data/DIALOGUE" + "/" + SONG.song.toLowerCase() + ".txt")){
					try{
						dialogue = CoolUtil.coolTextFile("assets/data/DIALOGUE" + "/" + SONG.song.toLowerCase() + ".txt");
						superShitterDialogue = true;
					}
					catch(e){}
				}
			case 'sen-beta':
				if(Assets.exists("assets/data/DIALOGUE2" + "/" + SONG.song.toLowerCase() + ".txt")){
					try{
						dialogue = CoolUtil.coolTextFile("assets/data/DIALOGUE2" + "/" + SONG.song.toLowerCase() + ".txt");
						superShitterDialogue = true;
					}
					catch(e){}
				}
			case 'sen-alpha':
				if(Assets.exists("assets/data/DIALOGUE3" + "/" + SONG.song.toLowerCase() + ".txt")){
					try{
						dialogue = CoolUtil.coolTextFile("assets/data/DIALOGUE3" + "/" + SONG.song.toLowerCase() + ".txt");
						superShitterDialogue = true;
					}
					catch(e){}
				}
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				lightChange = new FlxSprite(-320, -300).loadGraphic(Paths.image("stage_fade"));
				...
				light2.updateHitbox();
				light2 = new FlxSprite(1225, -100).loadGraphic(Paths.image('stage_light-other-right'));
				light2.scrollFactor.set(0.9, 0.9);
				add(light2);

			case 'spooky': //Week 2
				halloweenBG = new BGSprite('halloween_bg', -200, 100, ['halloweem bg0', 'halloweem bg lightning strike']); //Sorry low end pcs
				add(halloweenBG);
		...
		boyfriend = new Boyfriend(0, 0, CharMenuStory.bfmode);
		...
		if (SONG.song == 'Pumpd-up' || SONG.song == 'Pumpd-up(legacy)')
		{
			add(lightChange);
			add(stageAudience);
		}
		...
		{
			case 'Tutorial' | 'Tutorial(legacy)': 
				songcreator = "PolyField & GalXE";
			case 'Pumpd-up' | 'Pumpd-up(legacy)' | 'Decay' | 'Spokeebo':
				songcreator = "PolyField";
			case 'Westway' | 'Authority':
		...
		songnameTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 0, 400, "", 32);
		songnameTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		songnameTxt.scrollFactor.set();
		songnameTxt.alpha = 0;
		songnameTxt.borderSize = 2;
		songnameTxt.visible = showTime;
		if(ClientPrefs.downScroll) songnameTxt.y = FlxG.height - 44;
		songnameTxt.text = SONG.song;

		musiccreatorTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248,50 , 400, "", 32);
		...
		musiccreatorTxt.alpha = 0;
		musiccreatorTxt.borderSize = 2;
		musiccreatorTxt.visible = showTime;
		if(ClientPrefs.downScroll) musiccreatorTxt.y = FlxG.height - 94;
		...
		sickcounter = new FlxText(20, healthBarBG.y-60 , 0, "", 20);
		sickcounter.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		sickcounter.borderSize = 2;
		sickcounter.borderQuality = 2;
		sickcounter.scrollFactor.set();
		sickcounter.text = 'Sicks:' + sicksval;
		sickcounter.visible = !ClientPrefs.hideHud;
		if(!ClientPrefs.lowQuality) add(sickcounter); //Dude idk how haxe works lmao 
		...
		goodcounter.borderSize = 2;
		goodcounter.borderQuality = 2;
		goodcounter.scrollFactor.set();
		goodcounter.text = 'Goods:' + goodsval;
		goodcounter.visible = !ClientPrefs.hideHud;
		if(!ClientPrefs.lowQuality) add(goodcounter);
		...
		badcounter.borderSize = 2;
		badcounter.borderQuality = 2;
		badcounter.scrollFactor.set();
		badcounter.text = 'Bads :' + badsval;
		badcounter.visible = !ClientPrefs.hideHud;
		if(!ClientPrefs.lowQuality) add(badcounter);
		...
		shitcounter.borderSize = 2;
		shitcounter.borderQuality = 2;
		shitcounter.scrollFactor.set();
		shitcounter.text = 'Shits:' + shitsval;
		shitcounter.visible = !ClientPrefs.hideHud;
		if(!ClientPrefs.lowQuality) add(shitcounter);
		...
		misscounter.borderSize = 2;
		misscounter.borderQuality = 2;
		misscounter.scrollFactor.set();
		misscounter.text = 'Misses:' + songMisses;
		misscounter.visible = !ClientPrefs.hideHud;
		if(!ClientPrefs.lowQuality) add(misscounter);
		...
		sickcounter.cameras = [camHUD];
		goodcounter.cameras = [camHUD];
		badcounter.cameras = [camHUD];
		shitcounter.cameras = [camHUD];
		misscounter.cameras = [camHUD];
		songnameTxt.cameras = [camHUD];
		musiccreatorTxt.cameras = [camHUD];
	...
	function startCharacterPos(char:Character, ?gfCheck:Bool = false) {
		if(gfCheck && char.curCharacter.startsWith('lem')) { //IF DAD IS GIRLFRIEND, HE GOES TO HER POSITION
	...
	public static var superShitterDialogue2:Bool = false;
	...
	function startSong():Void
	{
		...
		/*if(Assets.exists("assets/data/DIALOGUE" + "/" + SONG.song.toLowerCase() + "-end.txt") && isStoryMode && storyWeek == 1){
			try{
				superShitterDialogue2 = true;
			}
			catch(e){}
		}
		if (superShitterDialogue2)
			FlxG.sound.music.onComplete = postDialogue;
		else if (superShitterDialogue2 == false)*/
		...
		FlxTween.tween(musiccreatorTxt, {alpha: 1}, 0.5, {ease: FlxEase.circOut});
	...
	override public function update(elapsed:Float)
	{
		...
		sickcounter.text = 'Sicks:' + ${sicks};
		goodcounter.text = 'Goods:' + ${goods};
		badcounter.text =  'Bads :' + ${bads};
		shitcounter.text = 'Shits:' + ${shits};
		misscounter.text = 'Combo Breaks:' + songMisses;

		callOnLuas('onUpdate', [elapsed]);

		Application.current.window.title = "Friday Night Flippin' SONG: " + curSong + ' | ' + scoreTxt.text;
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Rating: ' + ratingName + ' | Health: ' + healthBar.percent;
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' | Rating: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC + ' | Health: ' + healthBar.percent;
		}

		if(botplayTxt.visible && !inCutscene) {
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				Application.current.window.title = "Friday Night Flippin'";
				...
				if(SONG.song.toLowerCase() == "cheeseburger")
				{
					FlxG.switchState(new VideoGameOver());
				} 
				else openSubState(new GameOverSubstate(boyfriend.getScreenPosition().x - boyfriend.positionArray[0], boyfriend.getScreenPosition().y - boyfriend.positionArray[1], camFollowPos.x, camFollowPos.y));
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			case 'Hey!':
				...
				if(value != 0) {
					if(dad.curCharacter.startsWith('lem')) { //Tutorial GF is actually Dad! The GF is an imposter!! ding ding ding ding ding ding ding, dindinding, end my suffering
				...
				switch(charType) {
					...
					case 1:
						if(dad.curCharacter != value2) {
							...
							var wasGf:Bool = dad.curCharacter.startsWith('lem');
			...
			case 'Trigger bg lights': //Flippin events :)))
			...
			case 'Hide Main HUD':
				var boner:Float = Std.parseFloat(value1);
				if(Math.isNaN(boner)) boner = 0;		
				if (camHUD.alpha == 1)
				{
					FlxTween.tween(camHUD, {alpha: 0}, boner);
				}
				else if (camHUD.alpha < 0.9)
				{
					FlxTween.tween(camHUD, {alpha: 1}, boner);
				}
	...
	public function postDialogue():Void
	{
		canPause = false;
		FlxG.sound.music.volume = 0;
		vocals.volume = 0;
		doof.finishThing = endSong;
		dialogue = CoolUtil.coolTextFile("assets/data/DIALOGUE" + "/" + SONG.song.toLowerCase() + "-end.txt");
		//schoolOutro(doof);
	}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (SONG.song.toLowerCase() == 'fuckingkitty')
			{
				FlxG.save.data.beatTheKittysAss = true;
			}
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && !changedDifficulty && !usedPractice)
					{
						...
						{
							case 'tutorial':
								if(!GameJoltAPI.checkTrophy(167016))
									GameJoltAPI.getTrophy(167016, '#FEF200', 'Thats how you do it!', 'Beat tutorial with no misses');
							case 'week1':
								if(!GameJoltAPI.checkTrophy(167013))
									GameJoltAPI.getTrophy(167013, '#FEF200', 'Pumped', 'Beat week 1 with no misses');
							case 'week2':
								if(!GameJoltAPI.checkTrophy(167014))
									GameJoltAPI.getTrophy(167014, '#FEF200', 'Ayo why he ourple?', 'Beat week 2 with no misses');
						}
					}
					...
					else if (SONG.song.toLowerCase() == 'westway') 
					{
						switch (CharMenuStory.bfmode)
						{
							case 'sen':
								if (!FlxG.save.data.spookysen) {
									FlxG.save.data.spookysen = true;
								}
								if(!GameJoltAPI.checkTrophy(167009) && FlxG.save.data.spookysen && FlxG.save.data.pumpdsen)
									GameJoltAPI.getTrophy(167009, '#FEF200', 'True Love', 'Finish story Mode with Sen');
							case 'sen-beta':
								if (!FlxG.save.data.spookybeta) {
									FlxG.save.data.spookybeta = true;
								}
								if(!GameJoltAPI.checkTrophy(167010) && FlxG.save.data.spookybeta && FlxG.save.data.pumpdbeta)
									GameJoltAPI.getTrophy(167010, '#FEF200', '20 Dollars', 'Finish Story Mode with Beta');
							case 'sen-alpha':
								if (!FlxG.save.data.spookybeta) {
									FlxG.save.data.spookyalpha = true;
								}
								if(!GameJoltAPI.checkTrophy(167011) && FlxG.save.data.spookyalpha && FlxG.save.data.pumpdalpha)
									GameJoltAPI.getTrophy(167011, '#FEF200', 'Racism win', 'Finish Story Mode with Alpha');
						}

						if (!FlxG.save.data.alreadybeatstorymode)
						{
	...
	override function stepHit()
	{
		...
		if (curSong.toLowerCase() == 'pumpd-up' || curSong.toLowerCase() == 'pumpd-up(legacy)')
		{
			switch (curStep)
			{
				case 380:
					switch (CharMenuStory.bfmode)
					{
						case 'sen':
							ADDSHIT = ' STAGE';
							triggerEventNote('Alt Idle Animation', 'BF', '-alt');
						case 'sen-beta':
							ADDSHIT = ' STAGE';
						case 'sen-alpha':
							ADDSHIT = ' STAGE';
	...
	override function beatHit()
	{
		...
		switch (curIcon) // Icons can have animations now :))))
			{
				case 'deariez':
					if (curBeat % 2 == 0)
					{
						var tweentime:Float = 0.2;
						var angleRight:Float = 10;
						trace('deariez'+specialBool);
		
						specialBool = !specialBool;
		
						if (specialBool)
							FlxTween.tween(iconP2, {angle: angleRight*-1}, tweentime, {ease: FlxEase.quintOut});
						else
							FlxTween.tween(iconP2, {angle: angleRight}, tweentime, {ease: FlxEase.quintOut});
					}
